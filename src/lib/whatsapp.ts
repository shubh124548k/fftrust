...
/* ============================================================
 * SELLER INTAKE — structured intake data for WhatsApp prefill
 * Collects ONLY non-secret public listing information.
 * Never passwords, OTPs or recovery codes.
 * ============================================================ */
...
/**
 * Build a structured WhatsApp message from seller-intake data.
 * The seller reviews and presses Send — never automatic.
 */
export function buildSellerIntakeWhatsAppUrl(data: SellerIntakeData): string {
  const lines: string[] = [];
  lines.push("FF TRUST — Seller Intake");
  lines.push("");
  lines.push("— Listing details —");
  if (data.displayName) lines.push(`Display name: ${data.displayName}`);
  if (data.title) lines.push(`Title: ${data.title}`);
  if (typeof data.price === "number") lines.push(`Price: ₹${data.price} INR`);
  if (typeof data.level === "number") lines.push(`Level: ${data.level}`);
  if (data.rank) lines.push(`Rank: ${data.rank}`);
  if (typeof data.prime === "boolean") lines.push(`Prime: ${data.prime ? "Yes" : "No"}`);
  if (data.collections) lines.push(`Collections: ${data.collections}`);
  if (data.weapons) lines.push(`Weapons: ${data.weapons}`);
  if (data.evo) lines.push(`Evo: ${data.evo}`);
  if (data.description) lines.push(`Description: ${data.description}`);
  if (data.evidenceAvailability) lines.push(`Evidence: ${data.evidenceAvailability}`);
  if (data.contactPreference) lines.push(`Contact: ${data.contactPreference}`);
  if (data.terms) lines.push(`Terms: ${data.terms}`);
  lines.push("");
  lines.push("Submission is not verification and not publication.");
  lines.push("The owner reviews and manually publishes approved canonical records.");
  lines.push("");
  lines.push("(Sent via FF TRUST — independent platform.)");
